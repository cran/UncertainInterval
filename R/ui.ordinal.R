#' Function to calculate possible uncertain intervals of ordinal test results of individuals with (1) and without (0) the targeted condition.
#' @name ui.ordinal
#' @importFrom grDevices rgb
#' @importFrom graphics legend plot
#' @description This function calculates the uncertain interval (UI) of the test results of the two groups. This function is intended to be used for tests with 20 or less ordered test values. The lower range of test scores identifies patients without the targeted condition (lower More Certain Interval (MCI)), the upper interval of test scores above the uncertain interval identifies the patients with the condition (upper MCI). Due to the limited number of distinguishable scores, the estimations are course. When more than 20 values can be distinguished, \code{\link{ui.nonpar}} or \code{\link{ui.binormal}} may be preferred.
...
#' @param test The test pr predictor under evaluation. A column in a dataset or vector indicating the test results in a continuous scale. It is expected that true patients have higher scores than non-patients. If this is not the case, the test scores should be negated (test = -(test scores)).
#' @param select.max Selects the candidate thresholds on basis of a desired property of the More Certain Intervals (MCI). The criteria are: maximum Se+Sp (default), maximum C, maximum Accuracy, maximum Sp, maximum Se, maximum size of MCI. The last alternative 'All' is to choose all possible details.
#' @param constraints Sets upper constraints for various properties of the uncertain interval: C-statistic, Acc, lower and upper limit of the ratio of the proportions with and without the targeted condition. The default values are C = .57, Acc = .6, lower.ratio = .8, upper.ratio = 1.25. These values implement the desired uncertainty of the uncertain interval. The value of C is considered the most important and has the most restrictive default value. For Acc and C, the values closest to the desired value are found and then all smaller values are considered. The other two constraints are straightforward lower and upper limits of the ratio between the number of patients with and without the targeted disease. If you want to change these values, it is necessary to name all values. C = 1 or Acc = 1 excludes C respectively accuracy as selection criterion. If no solution is found, the best is showed together with  a warning message.
...
#' @param intersection (Default = NULL). Optional value to de used as value for the intersection. If no value is supplied, the intersection is calculated using  \code{get.intersection(ref = ref, test = test, model='ordinal').}
#' @param return.all (Default = FALSE). When TRUE $data.table and $uncertain.interval are included in the output.
#' @param ... Further parameters that can be transferred to the density function.
...
#'       \item{Loss: }{min(fnr + fpr) = min(1 - (Se + Sp -1)) = 1 - max(tpr - fpr)
#'       lower range ( < threshold): the summed number of false positives for each test score, divided by the number of persons that have received that test score.
#'       upper range ( >= threshold): the summed number of false negatives, divided by the number of persons that have received that test score.
#'       The Youden Loss is equal to 1-Youden.index.
#'       }
#'       \item{C: }{Concordance; equals AUROC (Area Under Receiving Operating Curve)}
#'       }
...
#'      }
#'   \item{$intersection}{The (rounded) intersection for the distributions of the two groups. Most often, these distributions have no true point of intersection and the rounded intersection is an approximation. Often, this equals the Maximized Youden threshold (see Schisterman 2005). Warning: When a limited range of scores is available, it is more difficult to estimate the intersection. Different estimates can easily differ plusminus 1. When using a non-rounded value (for example 16.1), the effective threshold for the uncertain area is round(intersection+.5), in the mentioned example: 16.1 becomes 17. }
...
#'       \item{UI.Sp: }{Specificity of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. This estimate is rough and dependent on the intersection and cannot be recommended as a criterion for a short, ordinal scale. }
#'       \item{UI.Se: }{Sensitivity of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. This estimate is rough and dependent on the intersection and cannot be recommended as a criterion for a short, ordinal scale.}
#'       \item{UI.Acc: }{Accuracy of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. This estimate is rough and dependent on the intersection and cannot be recommended as a criterion for a short, ordinal scale.}
#'       \item{UI.C: }{Concordance (AUROC) of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. Rule of thumb: <= .6}
#'       \item{UI.ratio: }{The ratio between the proportion of patients in the uncertain area with and without the condition. Closer to one is 'better', that is, more uncertain; 0.8 < UI.ratio < 1.25 as a rule of fist.}
...
#'       \item{MCI.C: }{Concordance (AUROC) of the test scores outside the uncertain interval. Closer to .5 is 'better', that is, more uncertain. Rule of thumb: <= .6}
...
#'       uncertain interval: the sum of the absolute differences in the number of people in the norm group d0 and the number of persons in the group with the targeted condition (d1) per test score, divided by the total number of persons.}
#'       upper MCI: the summed number of false negatives, divided by the number of persons that have received that test score.
#'       The Loss can be compared to the loss of the Youden threshold, provided that the intersection is equal to the Youden threshold. If necessary, this can be forced by attributing the value of the Youden threshold to the intersection parameter.
#'       }
#'   \item{$candidates: }{Candidates with a loss lower than the Youden loss which might be considered for the Uncertain Interval. The candidates are selected based on the constraints parameter, that defines the desired constraints of the uncertain area, and the select.max parameter, that selects the desired properties of the lower and upper More Certain Interval. }
#'   }
#'
...
#' @details
#'
#' Due to the limited possibilities of short scales, it is more difficult to determine a suitable uncertain interval when compared to longer scales. For any threshold determination, one needs a representative sample of sufficient size (200 or larger). If there are no test scores below the intersection in the candidate uncertain area, Sp of the Uncertain Interval (UI.Sp) is not available, while UI.Se equals 1.
#' The essential question is always whether the patients with the test scores inside the uncertain interval can be sufficiently distinguished. The candidate intervals are first selected on the properties of the uncertain interval. The defaults are C lower than .6, Acc lower than .6, and the ratio of proportions of persons with / without the targeted condition between .8 and 1.25. These criteria ensure that all candidates for the uncertain interval have insufficient accuracy. The second criterion is the desired property of the More Certain Intervals (see select.max parameter).
#' The model used is 'ordinal'. This model default for the adjust parameter send to the density function is 2, but you can enter another value such as adjust = 1.
#'
#' Discussion of the first example (please run the code first):
#' Visual inspection of the Mixed Barplot shows that distinguishing patients with and without the targeted condition is almost impossible for test scores 2, 3 and 4.
...
#'  \item{\eqn{\sum_{i=u+1}^{h} d0_{i}}{sum(d0[(u+1):h])}}{ is the loss in the upper More Certain Interval, that is, the total of False Positives, the number of patients without the targeted condition with a test score higher than \emph{u}.}
#'
#' Loss L is higher when the deviation from equality is higher in the uncertain area, higher when the number of False Negatives is higher, and higher when the number of False Positives is higher. The loss of a single threshold method equals 1 - its Accuracy.
...
#' The remaining cases outside the uncertain interval (2:4) show high C, Accuracy, Specificity and Sensitivity.
#'
#' }
...
#'             \code{\link{ui.nonpar}} or \code{\link{ui.binormal}} may be preferred when more than 20 values can be distinguished on the ordinal test scale.
#' @examples
