...
#' @description This function calculates the uncertain interval (UI) of the test results of the two groups. This function is intended to be used for tests with 20 or less ordered test values. The range of test scores identifies patients without the targeted condition (lower More Certain Interval (MCI)), the interval of test scores above the uncertain interval identifies the patients with the condition (upper MCI). Due to the limited number of distinguishable scores, the estimations are course. When more than 20 values can be distinguished, \code{\link{ui.nonpar}} or \code{\link{ui.binormal}} may be preferred.
...
#' @param test The test pr predictor under evaluation. A column in a dataset or vector indicating the test results in a continuous scale.
...
#' @param constraints Sets constraints for various properties of the uncertain interval: C-statistic, Acc, lower and upper limit of the ratio of the proportions with and without the targeted condition. The default values are C = .57, Acc = .6, lower.ratio = .8, upper.ratio = 1.25. These values implement the desired uncertainty of the uncertain interval. The value of C is considered the most important and has the most restrictive default value. For Acc and C, the values closest to the desired value are found and then all smaller values are considered. The other constraints are straightforward limits. If you want to change the values, it is recommended to name all values. C = 1 or Acc = 1 excludes C respectively accuracy as selection criterion.
...
#'       \item{UI.Sp: }{Specificity of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. As its estimate is dependent on the intersection, this is not recommended as a criterion. }
#'       \item{UI.Se: }{Sensitivity of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. As its estimate is dependent on the intersection, this is not recommended as a criterion.}
#'       \item{UI.Acc: }{Accuracy of the test scores between and including the lower and upper boundary. Closer to .5 is 'better', that is, more uncertain. As its estimate is dependent on the intersection, this is not recommended as a criterion.}
...
#'       \item{UI.ratio: }{The ratio between the proportion of patients in the uncertain area with and without the condition. Closer to one is better; 0.8 < UI.ratio < 1.25 as a rule of fist.}
...
ui.ordinal <- function(ref, test,
  ...
  tlim = c(C=.6, Acc=.6, lower.ratio=.8, upper.ratio=1.2)
  ...
  sel = ui$Loss <= Loss.Y & ui$UI.ratio <= constraints['upper.ratio'] &
    ui$UI.ratio >= constraints['lower.ratio']
  ui = ui[sel,]
